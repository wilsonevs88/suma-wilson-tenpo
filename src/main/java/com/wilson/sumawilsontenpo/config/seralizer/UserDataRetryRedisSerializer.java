...
public class UserDataRetryRedisSerializer implements RedisSerializer<UserDataRetryRedis> {

    private static final String CLASS_FIELD_VALUE = "@class";
    private static final String CLASS_CANONICAL_NAME = UserDataRetryRedis.class.getCanonicalName();
     ...
     * </pre>
     */
    public UserDataRetryRedisSerializer() {
     ...
     */
    @Override
    public byte[] serialize(UserDataRetryRedis userDataRedis) {
        try {
     ...
     */
    @Override
    public UserDataRetryRedis deserialize(byte[] bytes) {
        if (bytes == null) {
            return null;
        }
        ...
        try {
            return om.readValue(data, UserDataRetryRedis.class);
        } catch (JsonProcessingException jsonProcessingException) {
            throw new SerializationException(jsonProcessingException.getMessage(),
                    jsonProcessingException);
        }
