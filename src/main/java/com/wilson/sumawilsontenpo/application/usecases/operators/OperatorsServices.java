...
public class OperatorsServices implements OperadoresInputPort {
    ...
    public BaseUserResponse getUserId(String userId) {
        ...
        if (getUserIdRedis != null) {
            ...
            response = BaseUserResponse.builder()
                    ...
                    .responseContent(mapper.toUserDataRedis(UserDataRedis.builder()
                            .id(getUserIdData.getId())
    ...
    public BaseOperadoresResponse saveUser(OperatorsRequest request) {
        var response = BaseOperadoresResponse.builder().build();
        if (request.getValueUno() == null) {
            response = BaseOperadoresResponse.builder().responseCode(ResponseCode.NECESSARY_TO_ENTER_VALUE_ONE.getCode())
                    .responseDescription(ResponseCode.NECESSARY_TO_ENTER_VALUE_ONE.getDescription())
                    .build();
        }

        if (request.getValueDos() == null) {
            response = BaseOperadoresResponse.builder().responseCode(ResponseCode.NECESSARY_TO_ENTER_VALUE_TWO.getCode())
        ...
        while (retries < maxRetries) {
            try {
                ...
                if (getRedisKey == null) {
                    redis.set(clientId, sessionDurationMinutes,
                ...
                log.info("getRedisKey: {}", getRedisKey);
                log.info("responseMapper: {}", responseMapper);
                response = BaseOperadoresResponse.builder()
...
//                                .expiration(redis.getTime(responseMapper.getResponseContent().getClientUuid()))
            ...
            if (retries > 3) {
                response = BaseOperadoresResponse.builder().responseCode(ResponseCode.CONNECTION_PERCENTAGE_ERROR.getCode())
